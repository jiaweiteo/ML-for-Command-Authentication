...
##### Unique Normal Dataset generator
...
class Generator_unique_normal(Generator_Common):
	...
	def setup_log(self):
		...
		self.logger = logging.getLogger('generate_ss_datasets_unique_normal.py')
	...
	def initialize_options(self):
		...
		self.options = copy.deepcopy(Generator_unique_normal.initial_options_base)
	...
	def check_if_excluded_switch(self, idx):
		...
		for i in Generator_unique_normal.feeder_config_set.keys():
			for j in Generator_unique_normal.feeder_config_set[i]["switches"]:
				if idx in j: return True
			if idx in Generator_unique_normal.feeder_config_set[i]["couplers"]: return True
		
		# Check if switch is already part of transformer-related "options"
		for i in Generator_unique_normal.trafo_config_set.keys():
			if idx in Generator_unique_normal.trafo_config_set[i]["couplers"]: return True
		
		# Check if switch is one of the fixed switches
		if idx in Generator_unique_normal.static_switch_set: return True
	...
	def exercise_random_option(self):
		...
		if target_params[0]=="feeder":
			enable_list = self.get_guarantee_non_extreme_list(len(Generator_unique_normal.feeder_config_set[target]["switches"]))
			if "feeders" in Generator_unique_normal.feeder_config_set[target]:
				for i in range(len(enable_list)):
					self.net.ext_grid.at[Generator_unique_normal.feeder_config_set[target]["feeders"][i],"in_service"] = enable_list[i]
			for i in range(len(enable_list)):
				if enable_list[i]: continue
				interm_statuses = self.get_guarantee_min_1_list(len(Generator_unique_normal.feeder_config_set[target]["switches"][i]),False)
				for j in range(len(interm_statuses)):
					self.net.switch.at[Generator_unique_normal.feeder_config_set[target]["switches"][i][j],"closed"] = interm_statuses[j]
			for i in Generator_unique_normal.feeder_config_set[target]["couplers"]:
		...
		# In this case, for transformer "options", turn off 1 random transformer and enable corresponding output coupler
			...
			enable_list = self.get_guarantee_non_extreme_list(len(Generator_unique_normal.trafo_config_set[target]["trafos"]))
			for i in range(len(enable_list)):
				self.net.trafo.at[Generator_unique_normal.trafo_config_set[target]["trafos"][i],"in_service"] = enable_list[i]
			for i in Generator_unique_normal.trafo_config_set[target]["couplers"]:
				self.net.switch.at[i,"closed"] = True
...
# Main function - perform unique normal dataset generation
def main(output_file):
	m = Generator_unique_normal(output_file)
	m.main()
